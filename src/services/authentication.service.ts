...
export class AuthenticationService {
    ...
    private async getCompanyLogo(): Promise<string> {
        return await this.settingService.getConfigValue("default", 'companylogo');
    }
    ...
    async signUp(signUpDto: SignUpDto, activeUser: ActiveUserData = null): Promise<User> {
        ...
        if (!(await this.settingService.getConfigValue('iam', 'allowPublicRegistration')) && !activeUser) {
            throw new BadRequestException(ERROR_MESSAGES.PUBLIC_REGISTRATION_DISABLED);
        }

        try {
            const onForcePasswordChange = await this.settingService.getConfigValue('iam', 'forceChangePasswordOnFirstLogin');
            const activateUserOnRegistration = await this.settingService.getConfigValue('iam', 'activateUserOnRegistration');
            const defaultRole = await this.settingService.getConfigValue('iam', 'defaultRole');
    ...
    private async populateForSignup<T extends User>(user: T, signUpDto: SignUpDto, isUserActive: boolean = true, onForcePasswordChange?: boolean) {
        // const user = new User();
        let autoGeneratedPwdPermission = await this.settingService.getConfigValue('iam', 'iamAutoGeneratedPassword');
    ...
    private async notifyUserOnForcePasswordChange(user: User, autoGeneratedPwd: string) {
        ...
        mailService.sendEmailUsingTemplate(
            ...
            await this.settingService.getConfigValue("email", 'shouldQueueEmails'),
            null,
            null,
            'user',
            user.id
    ...
    private async isWelcomeEmailEnabled(): Promise<boolean> {
        const sendWelcomeEmailOnSignup = await this.settingService.getConfigValue('iam', 'sendWelcomeEmailOnSignup');
        return sendWelcomeEmailOnSignup;
    }

    private async isWelcomeSmsEnabled(): Promise<boolean> {
        const sendWelcomeSmsOnSignup = await this.settingService.getConfigValue('iam', 'sendWelcomeSmsOnSignup');
    ...
    private async notifyUserOnSignup(user: User) {
        ...
        // SMS welcome
        if (await this.isWelcomeSmsEnabled() && user.mobile) {
            this.smsService.sendSMSUsingTemplate(
                ...
                await this.settingService.getConfigValue("sms", 'shouldQueueSms'),
    ...
    async otpInitiateRegistration(signUpDto: OTPSignUpDto) {
        try {
            ...
            const passwordlessRegistrationValidateWhat = await this.settingService.getConfigValue('iam', 'passwordlessRegistrationValidateWhat');
            ...
            if (isEmpty(user)) {
                ...
                await this.userService.addRoleToUser(user.username, await this.settingService.getConfigValue('iam', 'defaultRole'));
            }
            else {
                await this.populateVerificationTokens(finalRegistrationVerificationSources, user);
                await this.userRepository.save(user);
    ...
    private async populateVerificationTokens(finalRegistrationValidationSources: string[], user: User) {
        ...
        if (finalRegistrationValidationSources.includes(TransactionalRegistrationValidationSource.EMAIL)) {
            ...
            const autoLoginUserOnRegistration = await this.settingService.getConfigValue('iam', 'autoLoginUserOnRegistration');
    ...
    private async notifyUserOnOtpInitiateRegistration(user: User, registrationValidationSources: string[]) {
        const companyLogo = await this.getCompanyLogo();
        const dummyOtp = await this.settingService.getConfigValue('iam', 'dummyOtp');
    ...
    async otpConfirmRegistration(confirmSignUpDto: OTPConfirmOTPDto) {
        ...
        if (confirmSignUpDto.type === RegistrationValidationSource.EMAIL) {
            ...
            user.active = await this.settingService.getConfigValue('iam', 'activateUserOnRegistration') && await this.areRegistrationValidationSourcesVerified(user);
    ...
    private async otp(): Promise<otp> {
        const now = new Date();
        const otpExpiry = await this.settingService.getConfigValue('iam', 'otpExpiry');
        const dummyOtp = await this.settingService.getConfigValue('iam', 'dummyOtp');
    ...
    private async generateForgotPasswordToken() {
        ...
        const forgotPasswordVerificationTokenExpiry = await this.settingService.getConfigValue('iam', 'forgotPasswordVerificationTokenExpiry');
        const dummyOtp = await this.settingService.getConfigValue('iam', 'dummyOtp');
    ...
    private async notifyUserOnForgotPassword(user: User) {
        ...
        const forgotPasswordSendVerificationTokenOn = await this.settingService.getConfigValue('iam', 'forgotPasswordSendVerificationTokenOn');
    ...
    async generateAccessToken(user: User) {
        ...
        const accessTokenTtl = await this.settingService.getConfigValue("jwt", "accessTokenTtl");
    ...
    async generateRefreshToken(user: User, previousRefreshToken?: string) {
        ...
        const refreshTokenTtl = await this.settingService.getConfigValue('jwt', "refreshTokenTtl");
    ...
    async refreshTokens(refreshTokenDto: RefreshTokenDto) {
        try {
            const secret = await this.settingService.getConfigValue("jwt", "secret");
            const audience = await this.settingService.getConfigValue("jwt", "audience");
            const issuer = await this.settingService.getConfigValue("jwt", "issuer");
    ...
    private async isPasswordlessRegistrationEnabled() {
        return this.settingService.getConfigValue('iam', 'passwordlessRegistration');
    }
