...
export class AuthenticationService {
  ...
  async signupForExtensionUser<T extends User, U extends CreateUserDto>(signUpDto: SignUpDto, extensionUserDto: U, extensionUserRepo: Repository<T>,): Promise<T> {
    try {
      ...
      //@ts-ignore
      const extensionUser = extensionUserRepo.merge(
        extensionUserRepo.create() as T,
        extensionUserDto as unknown as DeepPartial<T>,
      );
      var { user, pwd, autoGeneratedPwd } = await this.populateForSignup<T>(
        extensionUser,
        signUpDto,
        extensionUserDto.active ?? true,
        onForcePasswordChange,
      );
      const savedUser = await extensionUserRepo.save(user);

      await this.handlePostSignup(
        savedUser,
        signUpDto.roles,
        pwd,
        autoGeneratedPwd,
  ...
  private async buildLoginTokenResponse(user: User) {
    ...
    return {
      accessToken,
      refreshToken,
      user: { id, username, email, mobile, lastLoginProvider, roles },
    };
  }
