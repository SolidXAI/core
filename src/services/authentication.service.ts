...
export class AuthenticationService {
    ...
    async signUp(signUpDto: SignUpDto, activeUser: ActiveUserData = null): Promise<User> {
        ...
        try {
            ...
            await this.handlePostSignup(userRoles, user, pwd, savedUser, autoGeneratedPwd);
    ...
    async signupForExtensionUser<T extends User, U extends SignUpDto>(signUpDto: U, extensionUserRepo: Repository<T>): Promise<T> {
        try {
            // Merge the extended signUpDto attributes into the user entity
            //@ts-ignore
            const extensionUser = extensionUserRepo.merge(extensionUserRepo.create() as T, signUpDto);
            ...
            await this.handlePostSignup(signUpDto.roles, user, pwd, savedUser, autoGeneratedPwd);
    ...
    private async handlePostSignup(roles: string[]=[], user: User, pwd: string, savedUser: User, autoGeneratedPwd: string) {
        let userRoles = [];
