...
@Injectable()
export class MqDashboardTotalMessagesKpiProvider implements IDashboardWidgetDataProvider {
    constructor(
        private readonly mqMessageRepository: MqMessageRepository,
    ) { }

    name(): string {
        return "MqDashboardTotalMessagesKpiProvider";
    }

    help(): string {
        return "Returns total mq message count after applying dashboard variables.";
    }
    ...
    async getData(
        ...
        return {
            ...
            data: {
                value,
                label: widgetDefinition?.name ?? "Total Messages",
            },
        };
    }
}
